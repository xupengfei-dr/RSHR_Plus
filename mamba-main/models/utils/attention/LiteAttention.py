...
if __name__ == '__main__':
    ...
    layers_caa = nn.ModuleList([LiteMLA(in_channels=3, out_channels=3,dim=1, scales=(3,5)) for _ in range(2)])
    ...
    # caa =  LiteMLA(in_channels=3, out_channels=3,dim=1, scales=(3,5))
    with torch.no_grad():
        # attn = caa(img_tensor)
    ...
    processed_img.save("LiteMLA_x_2_+.jpg")  # 保存

    block = LiteMLA(in_channels=3, out_channels=3,dim=1, scales=(3,5))  # scales: 单尺度:(5,); 多尺度:(3,5)
    input1 = torch.rand(64, 3, 128, 128) # 输入 B C H W
    output = block(input1)
    print(output.size())