...
class BottleneckSelfAttentionModNoNorm(nn.Module):
    def __init__(self, dim, bottleneck_dim=192, num_heads=8, dropout=0.0):
        """
        Args:
            dim (int): 输入输出通道 C（如 768）。
            bottleneck_dim (int): 瓶颈维度 D，建议 C/16。
            num_heads (int): 自注意力头数。
            dropout (float): attention dropout。
        """
        super().__init__()
        # 瓶颈1：降维
        self.proj_down = nn.Linear(dim, bottleneck_dim, bias=False)
        # 自注意力
        ...
        # 瓶颈2：升维
        self.proj_up = nn.Linear(bottleneck_dim, dim, bias=False)
    ...
    def forward(self, x):
        ...
        """
        # 1. 直接降维
        h = self.proj_down(x)  # (B, N, D)

        # 2. 自注意力
        h_attn, _ = self.attn(h, h, h)  # (B, N, D)

        # 3. 升维 + 残差
        u = self.proj_up(h_attn)  # (B, N, C)
        return x + u
...
class BottleneckSelfAttentionModNoNorm_Mona(nn.Module):
    def __init__(self,
                 in_dim,
                 mid_dim,
                 factor=4):
        ...
        # RSFlashAttention
        self.rsf_model_config = FlashAttConfig()
        self.attn = RSFlashAttention(config=self.rsf_model_config, layer_idx=0, is_causal=False)
        # self.attn = nn.MultiheadAttention(
        #     embed_dim=mid_dim,
        #     num_heads=8,
        #     dropout=0.0,
    ...
    def forward(self, x):
        ...
        # print(project1.shape)
        # 2. 自注意力
        project1, _ = self.attn(project1, project1, project1)  # (B, N, D)
        # print(project1.shape)
        nonlinear = self.nonlinear(project1)
        nonlinear = self.dropout(nonlinear)
        project2 = self.project2(nonlinear)

        return identity + project2 * self.alpha
...
if __name__ == "__main__":
    ...
    mod = BottleneckSelfAttentionModNoNorm(dim=C, bottleneck_dim=D, num_heads=4)
    print(mod)
    out = mod(dummy)
    print(out.shape)  # torch.Size([64, 257, 768])
