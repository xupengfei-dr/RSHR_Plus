...
class Query_Relevance_Token_Selection_Guided_Network(nn.Module):
    """
    QRTS：Query-Relevance Token Selection Mechanism
    - Q: visual tokens (e.g., CLS or all patches)
    - K,V: text tokens
    创新点：
      1) Head-wise hq每个注意力头有一个“文本驱动”的查询向量head_queries以及语义相关性系数 hq_h ∈ (0,1)，用点积注意力从文本里提取，无MLP。
    ...
    def __init__(self, dim=768, num_heads=8, attn_drop=0.1, proj_drop=0.1,
        ...
        # 每个头一个可学习“查询原型”（不经MLP），用于从文本中估计相关性分布与语义系数
    ...
    def forward(self, x_q, x_kv, kv_attn_mask=None):
        ...
        # ====== 1) 每头hq：用 head_queries 与文本做点积注意力，得到 hq_h ∈ (0,1) ======
        # head_queries: [H, Dh] -> [B,H,1,Dh] 便于批处理
        hq = self.head_queries.unsqueeze(0).unsqueeze(2).expand(B, self.h, 1, self.dh)  # [B,H,1,Dh]
        # 对文本tokens做注意力：score_{h,t} = <head_queries_h, k_{h,t}> / sqrt(Dh)
        hq_scores = (hq * k).sum(-1) / (self.dh ** 0.5)  # [B,H,Lt]
        if kv_attn_mask is not None:
            mask = (kv_attn_mask == 0).unsqueeze(1)  # [B,1,Lt]
            hq_scores = hq_scores.masked_fill(mask, float('-inf'))

        gate_attn = hq_scores.softmax(dim=-1)  # [B,H,Lt]
        # 每头一个向量：c_h = Σ_t attn_h,t * k_h,t
        c_h = torch.einsum('bhl,bhld->bh d', gate_attn, k)  # [B,H,Dh]
        # 将 c_h 与 head_queries 做相似度，得到语义相关性系数 hq_h（标量）
        g_logits = (c_h * self.head_queries.unsqueeze(0)).sum(-1) / (self.dh ** 0.5)  # [B,H]
        hq_h = torch.sigmoid(g_logits).unsqueeze(-1).unsqueeze(-1)  # [B,H,1,1]

        # ====== 2) Top-k 文本路由（可选）：每个头仅保留最相关的 k 个文本token ======
        if self.topk and self.topk < Lt:
            # 用 hq_scores 作为相关度度量
            topk_val, topk_idx = torch.topk(hq_scores, k=self.topk, dim=-1)  # [B,H,K]
            # 构建mask
            keep_mask = torch.zeros_like(hq_scores, dtype=torch.bool)  # [B,H,Lt]
        ...
        # ====== 4) Head-wise hq调制 + 残差门 ======
        # 每头缩放
        out = out * hq_h  # [B,H,Lq,Dh]
