...
┡━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┩
│ Building_Condition_Recognition_accu… │           96.55172729492188           │
│      Complex_Counting_accuracy       │           44.08210372924805           │
│     Density_Estimation_accuracy      │           89.3382339477539            │
│ Entire_Image_Condition_Recognition_… │           98.09526062011719           │
│       Risk_Assessment_accuracy       │           96.76258850097656           │
│ Road_Condition_Recognition_accuracy  │           97.50503540039062           │
│       Simple_Counting_accuracy       │           48.58156204223633           │
│               test_acc               │          0.8608695864677429           │
│ test_acc_Building_Condition_Recogni… │           96.55172729492188           │
│      test_acc_Complex_Counting       │           44.08210372924805           │
│     test_acc_Density_Estimation      │           89.3382339477539            │
│ test_acc_Entire_Image_Condition_Rec… │           98.09526062011719           │
│       test_acc_Risk_Assessment       │           96.76258850097656           │
│ test_acc_Road_Condition_Recognition  │           97.50503540039062           │
│       test_acc_Simple_Counting       │           48.58156204223633           │
│              test_loss               │          0.5232375264167786           │
└──────────────────────────────────────┴───────────────────────────────────────┘
'''


class Conv_Attention_XT16(nn.Module):
    def __init__(self,
                 in_dim, mid_dim=384):
        super().__init__()
        # ------------------------EAT------------------------
        ...
        self.adapter_conv = Mu_Conv(mid_dim)
        self.project2 = nn.Linear(mid_dim, in_dim)

        # ----------------------RSFlashAttention 分支-------------------------
        self.attn_project_in = nn.Linear(in_dim, mid_dim)
        self.model_config_fla = FlashAttConfig(hidden_size=mid_dim)
        self.attn_flash_ = RSFlashAttention(
            config=self.model_config_fla,
            layer_idx=0,
            is_causal=False
        )
        ...
        # -------- Gradient-Decoupled Bidirectional Cooperation（新增协同） --------
        # 两个标量控制“卷积 <- 注意力”和“注意力 <- 卷积”的协同强度
        # 0 初始化 + tanh 约束，保证初始时完全等价于原结构，且扰动有限
        self.coop_c = nn.Parameter(torch.zeros(1))  # attn 引导 conv
        self.coop_a = nn.Parameter(torch.zeros(1))  # conv 引导 attn
    ...
    def forward(self, x, hw_shapes=None):
        """
        x: [B, 1 + N, C]，前面为 cls_token，后面为 patch token
        hw_shapes: (H, W)，满足 H * W == N
        """
        identity = x
        ...
        project1 = self.project1(conv_mid_)  # [B, N, mid_dim]
        ...
        project2 = self.project2(nonlinear)  # [B, N, C]

        conv_result_ = torch.cat([cls_token, project2], dim=1)  # [B, 1+N, C]

        # ------------------------注意力分支（全局）------------------------
        atten_in_ = self.attn_project_in(x)  # [B, 1+N, mid_dim]

        position_ids_input = torch.arange(
            atten_in_.shape[1], device=x.device
        ...
        atten_out = self.attn_project_out(atten_out)  # [B, 1+N, C]

        # -------- 梯度解耦的双向协同反馈（真正的“协同”在这里） --------
        # 思想：
        #   conv 分支在前向表示上向 attn 靠拢一点（获得更多全局语义），
        #   attn 分支在前向表示上向 conv 靠拢一点（注入更多局部结构），
        #   但这种“靠拢”是用对方的 detach 特征：只改表示，不改梯度流。
        #
        # 优点：
        #   - 卷积和注意力的参数梯度各自独立，训练稳定、几乎不受协同项影响；
        #   - 表示空间上两者互为“教师”，形成真正的 local-global 协同表征。

        # 将协同强度约束在 (-1, 1)，避免过大扰动
        lambda_c = torch.tanh(self.coop_c)  # 标量，控制 attn -> conv 的引导强度
        lambda_a = torch.tanh(self.coop_a)  # 标量，控制 conv -> attn 的引导强度

        # conv 受到“全局分支”的引导（但使用 detach，不改变 attn 的梯度）
        conv_guided = conv_result_ + lambda_c * (atten_out.detach() - conv_result_.detach())
        # attn 受到“局部分支”的引导（使用 detach，不改变 conv 的梯度）
        attn_guided = atten_out + lambda_a * (conv_result_.detach() - atten_out.detach())

        # 注意：由于 detach 的存在：
        #   d(conv_guided)/d(conv_result_) = I
        #   d(conv_guided)/d(atten_out)    = 0
        #   d(attn_guided)/d(atten_out)    = I
        #   d(attn_guided)/d(conv_result_) = 0
        # => 梯度流与原结构完全一致，只是前向表示发生了“软对齐”。

        # ------------------------残差融合（形式保持不变）------------------------
        out = identity + conv_guided * self.alpha + attn_guided * self.beta
        return out
...
if __name__ == "__main__":
    ...
    with torch.no_grad():  # 推理时不需要梯度
        with autocast(device_type='cuda', enabled=True, dtype=dtype):
            mona_model = Conv_Attention_XT15(768, mid_dim=256).to(device).to(dtype)
