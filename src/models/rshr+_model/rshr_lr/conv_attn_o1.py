...
class Conv_Attention(nn.Module):
    ...
    def forward(self, x, hw_shapes=None):
        ...
        position_ids_input = torch.arange(atten_in_.shape[1]).unsqueeze(0).to(x.device)  # Shape [1, seq_len]
        ...
        # return identity +  + atten_out * self.beta
        # return identity + conv_result_ * self.alpha
...
if __name__ == "__main__":
    ...
    with torch.no_grad():  # 推理时不需要梯度
        with autocast(device_type='cuda', enabled=True, dtype=dtype):
            mona_model = Conv_Attention(768,mid_dim=256).to(device).to(dtype)
