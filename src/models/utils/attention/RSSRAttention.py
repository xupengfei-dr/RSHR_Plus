...
if __name__ == "__main__":
    # --- 设置环境 ---
    torch.cuda.set_device(1)  # 尝试设置 GPU 1
    ...
    # --- 实例化模型 ---
    layer_idx_test = 0  # 指定层索引
    is_causal_test = False  # 设置是否为因果注意力 (例如用于 Encoder)
    print(f"Instantiating RSFlashAttention layer {layer_idx_test} (is_causal={is_causal_test})...")
    model = RSFlashAttention(config=model_config, layer_idx=layer_idx_test, is_causal=is_causal_test).to(device).to(
        dtype)  # 直接转换 dtype
    ...
    hidden_states_input = torch.rand(batch_size, seq_length, model_config.hidden_size, device=device, dtype=dtype)
    # 创建 position_ids (对于非缓存情况)
    position_ids_input = torch.arange(seq_length, device=device).unsqueeze(0)  # Shape [1, seq_len]
    ...
    try:
        with torch.no_grad():  # 推理时不需要梯度
            ...
            with autocast(device_type='cuda', enabled=True, dtype=dtype):  # 使用 autocast
                attn_output, _, _ = model(
                    hidden_states=hidden_states_input,
                    attention_mask=attention_mask_input,
                    position_ids=position_ids_input,  # 传递 position_ids
                    use_cache=False  # 本次测试不使用缓存
                )
        print("Forward pass finished successfully.")
        print(f"Output tensor shape: {attn_output.shape}")
        print(f"Output tensor dtype: {attn_output.dtype}")

        # --- 验证输出形状 ---
        expected_shape = (batch_size, seq_length, model_config.hidden_size)
        assert attn_output.shape == expected_shape, f"Expected shape {expected_shape}, but got {attn_output.shape}"
